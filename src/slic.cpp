	...
	const cv::Scalar&		color )
{
	...
	for( int j = 0; j < height; j++ )
	{
		for( int k = 0; k < width; k++ )
		{
			...
			if( np > 4 )//change to 2 or 3 for thinner lines
			{
	...
	const cv::Scalar&		color )
{
	...
	for( int j = 0; j < height; j++ )
	{
		for( int k = 0; k < width; k++ )
		{
			...
			if( np > 1 )//change to 2 or 3 for thinner lines
			{
				ubuff[mainindex] = (ushort)color.val[0];
				istaken[mainindex] = true;
			}
			mainindex++;
		}
	...
	const vector<double>&		edgemag)
{
	...
	if(perturbseeds)
	{
		PerturbSeeds(kseedsd,kseedsx, kseedsy, edgemag);
	}
}
	...
	const int&					NUMITR)
{
	...
	vector<double> sigmax(numk, 0);
	vector<double> sigmay(numk, 0);
	Mat sigmaprob = Mat::zeros(numk, NUM_PART, CV_64FC1);
	vector<int> clustersize(numk, 0);
	vector<double> inv(numk, 0);//to store 1/clustersize[k] values

	vector<double> distprob(sz, 100);
	vector<double> distxy(sz, DBL_MAX);
	vector<double> dT(sz, DBL_MAX);
	vector<double> distvec(sz, DBL_MAX);
	vector<double> maxlab(numk, 10*10);//THIS IS THE VARIABLE VALUE OF M, just start with 10
	vector<double> maxxy(numk, STEP*STEP);//THIS IS THE VARIABLE VALUE OF M, just start with 10
	...
	while( numitr < NUMITR )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
			...
			for( int y = y1; y < y2; y++ )
			{
				for( int x = x1; x < x2; x++ )
				{
					...
					{ 
						double prob_cur = (*prob).at<double>(i, ipart);
						double prob_seed = (*kseedprob).at<double>(n, ipart);
							
						//Debug
						/*if (y > 128 && y < 138 && x > 140 && x < 160)
						{
							//if ( kseedsd[n] == 0)
							{
								cout << "prob_seed: " << prob_seed << endl;
								cout << "prob_cur: " << prob_cur << endl;
							}
						}*/
						
						if (prob_cur > 0.000001 && prob_seed > 0.000001)
						{
							prob_cnt++;
							prob_tmp += prob_seed * log(prob_seed / prob_cur);
						}
					}

					if (prob_cnt > 0)
					{
						distprob[i] = fabs(prob_tmp);
					}
					else
					{
						distprob[i] = 100;
					...
					//------------------------------------------------------------------------
					double dist = distprob[i] + (distxy[i]*invxywt*gama);//only varying m, prettier superpixels
					...
					/*if (y>128 && y < 138 && x > 140 && x < 160)
					{
						//if ( kseedsd[n] == 0)
						{
							cout << "cnt : " << prob_cnt << endl;
							cout << "row: " << y << " col: " << x << endl;
							cout << "depth: " << l << endl;
							cout << "kseedsn[" << n << "]: " << kseedsd[n] << endl;
							cout << "kseedsx[" << n << "]: " << kseedsx[n] << endl;
							cout << "kseedsy[" << n << "]: " << kseedsy[n] << endl;
							cout << "dT[" << i << "]: " << dT[i] << endl;
							cout << "distprob[" << i << "]: " << distprob[i] << endl;
							cout << "distxy[" << i << "]: " << distxy[i] * invxywt*gama << "\n" << endl;
							getchar();
						}
					}*/
					
					
					//double dist = distxy[i];

					//------------------------------------------------------------------------
					if (dT[i] <= THRESHOLD_DT)
					{
						if (dist < distvec[i])
						{
							distvec[i] = dist;
		...
		{
			//_ASSERT(klabels[j] >= 0);
			//if the pixel has no label, assign the label which has the max number pixel in neighbour
			if (klabels[j] < 0)
			{
				int row = j/m_width, col = j % m_width;


				int label_cnt[N][2] = { { 0 } };

				int y1 = std::max(0,		(int)(row-offset_local));
				int y2 = std::min(m_height,	(int)(row+offset_local));
				int x1 = std::max(0,		(int)(col-offset_local));
				int x2 = std::min(m_width,	(int)(col+offset_local));
				...
				{
					for (int jcol = x1; jcol < x2; jcol++)
					{
						int coord = irow * m_width + jcol;
						int label_ = klabels[coord];
						...
						for (int t = 0; t < N; t++)
						{
							...
							if (label_cnt[t][0] != -1)
							{
								if (label_ == label_cnt[t][0])
								{
									label_cnt[t][1]++;
									if (max_cnt < label_cnt[t][1])
									{
										max_cnt = label_cnt[t][1];
										max_label = label_cnt[t][0];
									}
									break;
								}
							}
			...
			sigmad[klabels[j]] += m_lvec[j];
			sigmax[klabels[j]] += (j%m_width);
			sigmay[klabels[j]] += (j/m_width);
			for (int ipart = 0; ipart < NUM_PART; ipart++)
			{
				sigmaprob.at<double>(klabels[j], ipart) += (*prob).at<double>(j, ipart);
			}
		...
		{for( int k = 0; k < numk; k++ )
		{
			...
			for (int ipart = 0; ipart < NUM_PART; ipart++)
			{
				(*kseedprob).at<double>(k,ipart) = sigmaprob.at<double>(klabels[k], ipart) * inv[k];
			}
		}}
	...
	const double&				m)//weight given to spatial distance
{
	...
	PerformSuperpixelSegmentation_VariableSandM(kseedsl,kseedsa,kseedsb,kseedsx,kseedsy,klabels,STEP,m);
	numlabels = kseedsl.size();
	...
	const double&				m)//weight given to spatial distance
{
	...
	vector<double> edgemag(0);
	if(perturbseeds) DetectLabEdges(m_lvec, m_avec, m_bvec, m_width, m_height, edgemag);
	GetLABXYSeeds_ForGivenK(kseedsl,kseedsx, kseedsy, K, perturbseeds, edgemag);
	...
	const double&				m)//weight given to spatial distance
{
	...
	vector<double> edgemag(0);
	if(perturbseeds) DetectLabEdges(m_lvec, m_avec, m_bvec, m_width, m_height, edgemag);
	GetLABXYSeeds_ForGivenK(kseedsd,kseedsx, kseedsy, K, perturbseeds, edgemag);

	int dim = kseedsd.size();
	kseedsprob = Mat(dim, NUM_PART, CV_64FC1);
	//cout << "kseedsprob.rows " << kseedsprob.rows << endl;
	//cout << "prob.rows " << (*prob).rows << endl;
	for (int iDim = 0; iDim < dim; iDim++)
	{
		for (int j = 0; j < NUM_PART; j++)
		{
			int x = (int)(kseedsy[iDim] - 1) * m_width + (int)kseedsx[iDim];
		/*	cout << "kseedsx[iDim]: " << kseedsx[iDim] << endl;
			cout << "kseedsy[iDim]: " << kseedsy[iDim] << endl;
			cout << "x " << x << " j " << j << endl;
			*/
			kseedsprob.at<double>(iDim, j) = (*prob).at<double>(x, j);
		}
	}
	...
	int STEP = sqrt(double(sz)/double(K)) + 2.0;//adding a small value in the even the STEP size is too small.
	//PerformSuperpixelSLIC(kseedsl, kseedsa, kseedsb, kseedsx, kseedsy, klabels, STEP, edgemag, m);
	PerformSuperpixelSegmentation_VariableSandM(kseedsd,kseedsx,kseedsy,&kseedsprob,prob,klabels,STEP,m);
	numlabels = kseedsd.size();
...
void SLIC::GenerateSuperpixels(cv::Mat& img, UINT numSuperpixels, cv::Mat *prob, UINT m)
{
	if (img.empty()) {
		exit(-1);
	}
	...
	if (type == GRAY) {
		Mat2Buffer(img, bufferGray);
		PerformSLICO_ForGivenK(bufferGray, img.cols, img.rows, label, sz, numSuperpixels, m);
	}
	else if (type == RGB) {
		Mat2Buffer(img, bufferRGB);
		PerformSLICO_ForGivenK(bufferRGB, img.cols, img.rows, label, sz, numSuperpixels, m);
	}
	else if (type == GRAY_UINT16)
	{
		Mat2Buffer(img, bufferGray_UINT16);
		PerformSLICO_ForGivenK(bufferGray_UINT16, img.cols, img.rows, prob, label, sz, numSuperpixels, m);
	}
}
...
cv::Mat SLIC::GetImgWithContours(cv::Scalar color)
{
	...
	{
		DrawContoursAroundSegments(bufferGray_UINT16, label, m_width, m_height, color);
		cv::Mat result(m_height, m_width, CV_16UC1);
		memcpy(result.data, bufferGray_UINT16, m_width*m_height*sizeof(ushort));
		return result;
	}
}
...
{
	int row = m_height, col = m_width;
	Mat label_img = Mat::zeros(row, col, CV_16UC1);
	for (int i = 0; i < row; i++)
	{
		for (int j = 0; j < col; j++)
		{
			int index = i*col + j;
			label_img.at<ushort>(i, j) = label[index];
		}
	}
	imwrite("label.png", label_img);
}

cv::Mat SLIC::GetImgWithContours(cv::Mat img, cv::Scalar color)
{
		if (img.empty()) {
			exit(-1);
		}
...
void SLIC::Mat2Buffer(const cv::Mat& img, ushort*& buffer)
{
	int sz = img.cols * img.rows;
	if (bufferGray_UINT16) {
		delete [] bufferGray_UINT16;
	}
	bufferGray_UINT16    = new ushort[sz];

	memcpy( bufferGray_UINT16, (UINT*)img.data, sz*sizeof(ushort) );
